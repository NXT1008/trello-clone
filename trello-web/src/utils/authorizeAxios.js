...
* Giải pháp: Inject store: là kỹ thuật khi cần sử dụng biến redux store ở các file
    ngoài phạm vi react component như file authorizeAxios hiện tại
* Hiểu đơn giản: khi ứng dụng bắt đầu chạy lên, code sẽ chạy vào main.jsx đầu tiên,
    từ bên đó chúng ta gọi hàm injectStore ngay lập tức để gán biến mainStore vào biến
    axiosReduxStore cục bộ trong file này.
...
// Khởi tạo một đối tượng Axios (authorizedAxiosInstance) mục đích để
//  custom và cấu hình chung cho dự án.
...
//  (phục vụ việc chúng ta sẽ lưu JWT tokens (refresh & access)
//  vào trong httpOnly Cookie của trình duyệt)
...
// Mục đích tạo Promise này để khi nào gọi api refresh_token xong xuôi thì mới retry
//  lại nhiều api bị lỗi trước đó.
...
authorizedAxiosInstance.interceptors.response.use( (response) => {
  ...
  const originalRequests = error.config // những requests bị lỗi

  // if (error.response?.status === 410 && !originalRequests._retry) {
  if (error.response?.status === 410 && originalRequests) {
    // UPDATE THÊM: Có thể bỏ không cần thêm cái _retry giống như nhiều bài hướng dẫn khác
    //  trên mạng nữa vì chúng ta đang làm chuẩn với biến refreshTokenPromise ở trên rồi,
    // Gán thêm một giá trị _retry luôn = true trong khoảng thời gian chờ, đảm bảo việc
    //  refresh token này chỉ luôn gọi 1 lần tại 1 thời điểm (nhìn lại điều kiện if ngay phía trên)
    // originalRequests._retry = true

    // Trường hợp 2 > Bước 2: Kiểm tra xem nếu chưa có refreshTokenPromise thì thực hiện
    //  gán việc gọi api refresh_token đồng thời gán vào cho cái refreshTokenPromise
    if (!refreshTokenPromise) {
      // Thực hiện gán dữ liệu cho biến refreshTokenPromise
      refreshTokenPromise = refreshTokenAPI()
        .then(data => {
          /**
          * Đối với Trường hợp nếu dự án cần lưu accessToken vào localstorage hoặc
          *   đâu đó thì sẽ viết thêm code xử lý ở đây.
          * Hiện tại ở đây không cần làm gì vì đồng thời accessToken đã nằm trong
          *   httpOnly cookie (xử lý từ phía BE) sau khi api refreshToken được gọi thành công.
          */
        ...
        .finally(() => {
          // Dù API refresh_token có thành công hay lỗi thì vẫn luôn gán lại cái
          //  refreshTokenPromise về null như ban đầu
    ...
    return refreshTokenPromise.then(accessToken => {
      /**
      * Case 1: Đối với Trường hợp nếu dự án cần lưu accessToken vào localstorage hoặc
      *   đâu đó thì sẽ viết thêm code xử lý ở đây.
      ...
      */
      ...
      // Case 2: Bước Quan trọng: Return lại axios instance của chúng ta kết hợp các
      //  originalRequests để gọi lại những api ban đầu bị lỗi
