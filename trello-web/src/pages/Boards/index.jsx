...
function Boards() {
  ...
  // Hàm chung để thực hiện việc update boardList khi call API thành công
  ...
  useEffect(() => {
    ...
    //  lấy từ hook useLocation của react-router-dom cũng thay đổi theo, đồng nghĩa hàm
    //  useEffect sẽ chạy lại và fetch lại API theo đúng page mới vì cái localtion.search
    //  đã nằm trong dependencies của useEffect
    // console.log(location.search)

    const searchPath = location.search

    // Gọi API lấy danh sách boards ở đây...
    fetchBoardsAPI(searchPath).then(updateStateData)
  }, [location.search])

  const afterCreateNewBoard = () => {
    // Đơn giản là fetch lại danh sách board tương tự useEffect
    const searchPath = location.search
    fetchBoardsAPI(searchPath).then(updateStateData)
  }
