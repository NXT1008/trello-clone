...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMembers }) {
  ...
  // Lấy ra activeBoard từ redux ra, mục đích là lấy được toàn bộ thông tin những thành viên của board thông qua field: FE_allUsers
  ...
  // ? Trường hợp muốn hiển thị đúng thứ tự members trong card
          ...
          {board.FE_allUsers.map((user, index) =>
            <Tooltip title={user?.displayName} key={index}>
              ...
              <Badge
                ...
                badgeContent={
                  cardMemberIds.includes(user._id) // Nếu user là member của card
